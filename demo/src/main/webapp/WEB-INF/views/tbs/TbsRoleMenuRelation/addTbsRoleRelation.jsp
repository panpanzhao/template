	...
	$.messager.confirm("提示", "是否确认添加关联?", function(r){
		if (r){
			...
			$.each(rows,function(index,row){
				var obj={
					"roleId":row.tbsRoleId,
					"menuId":"${tbsMenu.tbsMenuId}"
				}
				rowList.push(obj);
			});
...
<body id="indexLayout" class="easyui-layout" >
    <div data-options="region:'west'" style="width:45%">
		<table id="left_table" class="easyui-datagrid" title="未选列表信息"
		       data-options="url:'${contextPath}/tbs/tbsRoleMenuRelation/queryUnCheckTbsRole?id=${tbsMenu.tbsMenuId}',
		       				striped:true">
			<thead>
				<tr>
					<th data-options="field:'_operate',checkbox:true">操作</th>
					<th data-options="field:'tbsRoleId',hidden:true" >角色主键</th>
    ...
    <div data-options="region:'east'" style="width:45%">
		<table id="right_table" class="easyui-datagrid" title="已选列表信息"
		       data-options="url:'${contextPath}/tbs/tbsRoleMenuRelation/queryCheckTbsRole?id=${tbsMenu.tbsMenuId}',
		       				striped:true">
			<thead>
				<tr>
					<th data-options="field:'_operate',checkbox:true">操作</th>
