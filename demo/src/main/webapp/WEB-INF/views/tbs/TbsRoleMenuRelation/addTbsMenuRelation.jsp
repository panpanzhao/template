...
function deleteRow(){
	...
	$.messager.confirm("提示", "是否取消关联?", function(r){
		if (r){
			...
			$.ajax({
				type: "POST",
				url: "${contextPath}/tbs/tbsRoleMenuRelation/deleteList",
				data:dataParem,
...
function saveRow(){
	...
	$.messager.confirm("提示", "是否确认添加关联?", function(r){
		if (r){
			...
			$.ajax({
				type: "POST",
				url: "${contextPath}/tbs/tbsRoleMenuRelation/addList",
				data:dataParem,
				contentType:"application/json;charset=UTF-8",
				success: function(msg){
					if(msg=="SUCCESS"){
...
<body id="indexLayout" class="easyui-layout" >
    <div data-options="region:'west'" style="width:45%">
		<table id="left_table" class="easyui-datagrid" title="未选列表信息"
		       data-options="url:'${contextPath}/tbs/tbsRoleMenuRelation/queryUnCheckTbsMenu?id=${tbsRole.tbsRoleId}',
		       				striped:true">
			<thead>
				<tr>
					<th data-options="field:'_operate',checkbox:true">操作</th>
					<th data-options="field:'tbsMenuId',hidden:true" >权限主键</th>
					<th data-options="field:'menuDesc'" >菜单描述</th>
    ...
    <div data-options="region:'east'" style="width:45%">
		<table id="right_table" class="easyui-datagrid" title="已选列表信息"
		       data-options="url:'${contextPath}/tbs/tbsRoleMenuRelation/queryCheckTbsMenu?id=${tbsRole.tbsRoleId}',
		       				striped:true">
			<thead>
				<tr>
					<th data-options="field:'_operate',checkbox:true">操作</th>
					<th data-options="field:'id',hidden:true" >角色权限关系主键</th>
					<th data-options="field:'menuDesc'" >菜单描述</th>
					<th data-options="field:'menuName'" >菜单名称</th>
					<th data-options="field:'parentId'" >父ID</th>
				</tr>
