...
<script type="text/javascript">
	//搜索列表
	function doSearch(){
		$('#table').datagrid('load',{roleId:$('#search_roleId').val(),menuId:$('#search_menuId').val()});
	}	
	//跳转添加页面
	function doAdd(){
		...
		$("#addIframe").attr("src","${contextPath}/tbs/tbsRoleMenu/add");
	...
	function doEdit(){
		...
		if($("#editWindow").length<=0){
			...
			editForm.css({"display":"none"}).attr({"action":"${contextPath}/tbs/tbsRoleMenu/edit","method":"post","target":"editIframe"});
		}
		$("#editForm").empty();
	...
	function doDelete(){
		...
		$.messager.confirm('提示', '是否确认删除?', function(r){
			if (r){
				...
				$.ajax({
					type: "POST",
					url: "${contextPath}/tbs/tbsRoleMenu/deleteList",
					data:dataParem,
...
<div id="paramSearch" >
	<ul class="searchUl">
		...
		<li style="">
			...
			<span>
				<input id="search_menuId" class="easyui-combotree" data-options="editable:true,url:'${contextPath}//tbs/tbsMenu/queryTree?showId=id&showText=menuName&treeId=id&treePId=parentId&treePValue='">
			</span>
		</li>
		<li style="display: ">
...
<table id="table" class="easyui-datagrid" title="列表信息"
       data-options="url:'${contextPath}/tbs/tbsRoleMenu/query',
       				toolbar:'#paramSearch',
       				striped:true,
	...
	<thead>
		<tr>
			...
			<th data-options="field:'id',sortable:true,hidden:true" >角色权限关系主键</th>
